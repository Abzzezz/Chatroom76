...
public class GotoCommand implements Command {
    ...
    public void execute(String in) {
        ...
        if (arguments.length == 1) {
            ...
            OPEN_GL_CORE_INSTANCE.getScreen().getOptions().stream().filter(option -> option.getTitle().equalsIgnoreCase(optionTitle) && option.getOptionType() == OptionType.SCREEN).findAny().ifPresent(option -> {
