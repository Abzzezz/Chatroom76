...
public class DialogHandler {
    ...
    private void prepareDialog() {
        ...
        dialog.stream().filter(s -> s.startsWith(BACKGROUND_MUSIC_CALL)).findAny().ifPresent(s -> {
            ...
            SoundPlayer.SOUND_PLAYER.playBackgroundMusic(backgroundMusic, volume);
            dialog.remove(s);
        });
    ...
    public void download(final String input, final Consumer<Integer> totalBytes, final Consumer<Integer> downloadedBytes, final Consumer<String> downloadingFile) {
        new Thread(() -> {
            ...
            try {
                ...
                DialogLoader.DIALOG_LOADER.addDialog(dialog);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }).start();
    ...
    public void loadDialog(final int index) {
        if (!MathUtil.inMaxBound(index, DialogLoader.DIALOG_LOADER.getDialogs().size())) return;

        final Dialog newDialog = DialogLoader.DIALOG_LOADER.getDialogs().get(index);
    ...
    public void deleteDialog(final int index) {
        if (!MathUtil.inMaxBound(index, DialogLoader.DIALOG_LOADER.getDialogs().size())) return;
        final Dialog removed = DialogLoader.DIALOG_LOADER.removeDialog(index);
